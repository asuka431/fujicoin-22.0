# PSBT Howto for Baricoin Core

Since Baricoin Core 0.17, an RPC interface exists for Partially Signed Baricoin
...
PSBT is an interchange format for Baricoin transactions that are not fully signed
...
#### Multisig with multiple Baricoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Baricoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `baricoin-cli` is used.
...
Later, when *V* BARI has been received on *Amulti*, and Bob and Carol want to
