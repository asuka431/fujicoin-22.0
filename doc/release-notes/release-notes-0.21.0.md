...
Baricoin Core version 0.21.0 is now available from:
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Baricoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Baricoin Core will no longer
...
- A new `baricoin-cli -netinfo` command provides a network peer connections
...
- A new `baricoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `baricoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `baricoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Baricoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `baricoin-wallet` tool which performs the salvage
...
Baricoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`baricoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Baricoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `baricoin-cli createwallet` or `baricoin-wallet
...
Users who already have a Baricoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Baricoin Core.
...
The change to using descriptors changes the default derivation paths used by Baricoin Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BARI per 1,000
  vbytes (BARI/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BARI/kB
  to sat/vB and the help documentation in BARI/kB is updated to BARI/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BARI/kvB to sat/vB and the
  ...
  difference between BARI/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BARI/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Baricoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `baricoin.conf`
...
- #19284 Add seed.baricoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in baricoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add baricoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in baricoin-cli (laanwj)
...
### Build system
- #18504 Drop baricoin-tx and baricoin-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in BARICOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for baricoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Baricoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrebaricoin)
...
- #18817 Document differences in baricoind and baricoin-qt locale handling (practicalswift)
