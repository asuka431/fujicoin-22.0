...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `baricoind` or `baricoin-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
`./`               | `banlist.json`        | Stores the addresses/subnets of banned nodes.
`./`               | `baricoin.conf`        | User-defined [configuration settings](baricoin-conf.md) for `baricoind` or `baricoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [baricoin.conf](baricoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
These subdirectories and files are no longer used by Baricoin Core:
