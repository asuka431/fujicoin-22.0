...
class AvoidReuseTest(BaricoinTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 BARI with
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 101 outputs of 1 BARI in the same reused
        address and tries to send a small payment of 0.5 BARI. The wallet
        should use 100 outputs from the reused address as inputs and not a
        single 1 BARI input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 101 outputs of 1 BARI to the same, reused address in the wallet
        for _ in range(101):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 202 outputs of 1 BARI in the same reused
        address and tries to send a payment of 200.5 BARI. The wallet
        ...
        # Send 202 outputs of 1 BARI to the same, reused address in the wallet
        for _ in range(202):
            self.nodes[0].sendtoaddress(new_addr, 1)
