...
To build Baricoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Baricoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/baricoin-detached-sigs> ./contrib/guix/guix-codesign
```
